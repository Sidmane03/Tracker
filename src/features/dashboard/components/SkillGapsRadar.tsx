  ...
  return (
    <div className="rounded-[20px] border border-white/[0.08] bg-[#10192b] p-5 sm:p-6">
      ...
      <div className="mt-4 space-y-1">
        {gaps.length === 0 ? (
          <div className="py-6 text-center text-xs text-[#8190aa]">
            No critical skill gaps detected. You are on track!
          </div>
        ) : (
          gaps.map(({ subtopic, categoryTitle }, index) => {
            const priority =
              (subtopic.weight ?? 1) >= 3 ? 'Critical' : (subtopic.weight ?? 1) === 2 ? 'High' : 'Medium'
            const dotColor =
              index === 0 ? 'bg-[#ed8ca7]' : index < 3 ? 'bg-[#f4bd78]' : 'bg-[#75bfff]'
            ...
            return (
              ...
              >
                ...
                >
                  <p className="truncate text-sm font-medium text-white group-hover:text-[#c0c7ff] transition">
                    {subtopic.title}
                  </p>
      ...
      <button
        ...
        className="mt-4 text-xs font-bold text-[#aab0ff] hover:text-white transition cursor-pointer"
      >
        Review all gaps →
      </button>
    </div>
